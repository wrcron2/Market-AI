      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className={`mf-chip ${enabled ? 'bg-signal-yellow/18 text-signal-yellow' : 'bg-surface-sunken text-ink-faint'}`}>
              {enabled ? 'AUTO ON' : 'AUTO OFF'}
            </span>
          </div>
          <p className="mt-2 text-[12.5px] leading-relaxed text-ink-muted">
            When enabled, the system places orders automatically during market hours{' '}
            <strong className="text-signal-orange">without your approval</strong>. This is the only place this can be changed.
          </p>
        </div>
        <button
          role="switch"
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 border-b border-line-soft px-5 py-4">
              ...
              <div>
                <div className="text-[16px] font-bold">Enable AUTO_EXECUTE</div>
                <div className="text-xs text-ink-muted">Autonomous order placement</div>
              </div>
            </div>
            <div className="px-5 py-5">
              <div className="mb-4 rounded-lg border border-signal-yellow/25 bg-signal-yellow/8 px-3.5 py-3 text-[13px] leading-relaxed text-signal-yellow">
            ...
            <div className="flex justify-end gap-2.5 border-t border-line-soft px-5 py-3.5">
              <button
                onClick={() => setModal(false)}
                className="rounded-lg border border-line bg-surface-hover px-4 py-2.5 text-[13px] font-semibold text-ink-muted hover:bg-[#2a2c3a]"
              >
                Cancel
              </button>
              <button
                ...
                className="rounded-lg bg-signal-yellow px-4 py-2.5 text-[13px] font-bold text-base hover:bg-signal-yellow/80 disabled:cursor-not-allowed disabled:bg-[#3f424d] disabled:text-ink-faint"
              >
